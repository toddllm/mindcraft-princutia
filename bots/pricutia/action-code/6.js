        ...
        async function replaceWaterInSection(startX, startZ) {
            for (let x = 0; x <= length; x++) {
                for (let z = 0; z <= width; z++) {
                    let offsetX = startX + x - (length / 2); if(bot.interrupt_code) {log(bot, "Code interrupted.");return;}
                    let offsetZ = startZ + z - (width / 2); if(bot.interrupt_code) {log(bot, "Code interrupted.");return;}
